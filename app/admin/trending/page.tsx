        ...
        <div>
            ...
            >
                {trendingItems.map((item, index) => (
                    <motion.div
                        ...
                        className="bg-premium-charcoal rounded-xl shadow-md border border-gray-800 overflow-hidden group"
                    >
                        <div className="h-48 bg-gray-900 relative overflow-hidden">
                            ...
                            <div className="absolute top-2 right-2 bg-black/80 backdrop-blur-sm px-2 py-1 rounded-full text-xs font-bold text-premium-gold border border-premium-gold/30">
                                {item.category}
                            </div>
                        </div>
                        <div className="p-4">
                            ...
                            <div className="flex justify-between items-center mt-4">
                                <button
                                    ...
                                    className={`flex items-center gap-1 text-sm font-medium ${item.isActive ? 'text-green-400' : 'text-gray-500'}`}
                                >
                                    {item.isActive ? <ToggleRight className="h-5 w-5" /> : <ToggleLeft className="h-5 w-5" />}
                                    {item.isActive ? 'Active' : 'Inactive'}
                                </button>
                                <div className="flex gap-2">
                                    <button onClick={() => handleEdit(item)} className="p-2 text-blue-400 hover:bg-blue-900/30 rounded-lg transition-colors">
                                        <Edit className="h-4 w-4" />
                                    </button>
                                    <button onClick={() => handleDelete(item.id)} className="p-2 text-red-400 hover:bg-red-900/30 rounded-lg transition-colors">
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setIsModalOpen(false)}>
                    <div className="bg-premium-charcoal border border-gray-800 rounded-2xl p-6 max-w-md w-full" onClick={e => e.stopPropagation()}>
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-xl font-bold text-white font-playfair">{editingItem ? 'Edit Item' : 'Add New Item'}</h2>
                            <button onClick={() => setIsModalOpen(false)}><X className="h-6 w-6 text-gray-400 hover:text-white transition-colors" /></button>
                        </div>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-400 mb-1">Category (Tag)</label>
                                <select
                            ...
                            <button type="submit" className="w-full py-3 bg-premium-gold hover:bg-white text-premium-black font-bold rounded-lg shadow-lg shadow-premium-gold/20 transition-all">
